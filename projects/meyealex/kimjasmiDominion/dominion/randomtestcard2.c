...
int testGreatHallCardEffect(int handPos, struct gameState *post) {
    ...
    if(post->discardCount[player] != pre.discardCount[player]+1)
        return 1;
    if(post->hand[player][handPos] == great_hall)
        return 1;
    return 0;
...
int main(int argc, char *argv[]) {
    ...
    for(n = 0; n < 2000; n++) {
        ...
        state.hand[p][handPos] = great_hall;
        state.playedCardCount = 0;
        state.numActions = floor(Random() * 10);
        r = testGreatHallCardEffect(handPos, &state);
        numFails += r;
    }
